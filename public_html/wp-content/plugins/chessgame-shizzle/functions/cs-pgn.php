...
/*
 * Generate a FEN code from the start position in a game and return it.
 *
 ...
 *
 * @param  int    $id     Post ID of the chessgame.
 * @param  string $pgn    PGN data which might have a FEN field.
 * @return string $cs_fen FEN code that was saved. Empty string if not found.
 *
 * @since 1.2.9
 */
function chessgame_shizzle_save_fen_to_meta( $id, $pgn ) {

	$cs_fen = chessgame_shizzle_pgn_get_fen( $pgn );
	$cs_new_fen = chessgame_shizzle_get_new_fen(); // start position.
	if ( strlen($cs_fen) > 0 && strlen($cs_new_fen) > 0 && $cs_fen !== $cs_new_fen ) {
		// FEN code found and it is not the start position.
		$cs_fen = chessgame_shizzle_sanitize_pgn( $cs_fen );
		update_post_meta( $id, 'cs_chessgame_fen', $cs_fen );
		return $cs_fen;
	}
