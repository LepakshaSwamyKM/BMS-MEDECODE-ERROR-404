...
class LIFEPRESS_AJAX{
	...
	// initial load data and templates
		function init_load(){
			...
			$st = new DateTime($sY.'-'.$sM.'-1');
			$this_tz = LIFEPRESS()->time->timezone;
			$st->setTimezone( $this_tz );
			
		
			// set start date to start of the week if sent date is 1st day of month
			//$st->modify( '-'. $this->_get_sow_daydif( $st->format('w')).'day');

			$J['d']['debug'] = $st->format('Y-m-d').' '. $this_tz->getOffset($st);
			...
			$st->modify( '+1 month');
			$st->modify( '+'. $this->_get_days_in_week_after( $st->format('w')).'day');

			$E = $st->format('U');		
			
			$ENT = new LIFEPRESS_Entries();

			$J['entry'] = $ENT->get_entries( $S, $E);
			$J['temp'] = array(				
			...
			// day names
				$J['d']['day_names'] = 
					array(
					    0=> array('f'=> __('Sunday','lp'), 's'=> __('Sun','lp') ),
					    array('f'=> __('Monday','lp'), 's'=> __('Mon','lp') ),
					    array('f'=> __('Tuesday','lp'), 's'=> __('Tue','lp') ),
					    array('f'=> __('Wednesday','lp'), 's'=> __('Wed','lp') ),
					    array('f'=> __('Thursday','lp'), 's'=> __('Thu','lp') ),
					    array('f'=> __('Friday','lp'), 's'=> __('Fri','lp') ),
					    array('f'=> __('Saturday','lp'), 's'=> __('Sat','lp') )
					 );
			// month names
				$J['d']['mo_names'] = array(
				    0=> __('January','lp'),
			...
			echo json_encode( apply_filters('lifepress_ajax_init_load', $J ) ); exit;
		}
		...
		function delete_item(){
			...
			if($res){
				echo json_encode(array(
					...
					'notice_msg'=>__('Successfully deleted '. $d['item_type']),					
				));exit;
			}else{
				echo json_encode(array(
					'status'=>'bad', 'notice_type'=>'bad',
					'notice_msg'=>__('Could not delete '.$d['item_type']),					
				));exit;
			}
		}
	...
	// get an item data
		public function get_item_data(){
			if( !isset($_POST['item_id']) || !isset($_POST['item_type'])){
				echo json_encode(array(
					'status'=>'bad', 'notice_type'=>'bad',
					'notice_msg'=>__('Missing Required Data'),					
				));exit;
			}
